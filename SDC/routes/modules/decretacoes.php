...
Route::prefix('decretacoes')->name('decretacoes.')->group(function () {
    ...
    // Create - Novo processo (ANTES da rota {id} para não conflitar)
    Route::get('/create', \App\Modules\Decretacoes\Presentation\Http\Controllers\ProcessoCreateController::class)->name('create');

    // Store - Salvar novo processo (POST)
    Route::post('/', \App\Modules\Decretacoes\Presentation\Http\Controllers\ProcessoStoreController::class)->name('store');
