 ...
 *
 * Nomenclatura `cisterna_*`. O banco tem modulos com prefixo curto (rat_*,
 * pae_*, tdap_*) e outros com o nome inteiro (compdec_*, ajuda_h_*, treinamento_*);
 * aqui vale o nome inteiro, que e o que o modulo ja usava.
 *
 * ADITIVA E IDEMPOTENTE, de proposito. Nao ha DROP no up(): cada criacao tem
 * guarda de hasTable, e indices e sequence usam IF NOT EXISTS. Roda com
 * seguranca sobre banco de producao e pode ser repetida.
 *
 * Por que uma migration NOVA em vez de reescrever a do scaffold (regra 9):
 * `2026_05_08_140000_create_cisternas_table` JA CONSTA como aplicada nos
 * ambientes onde o scaffold rodou. O Laravel nao reexecuta migration
 * registrada, entao reescreve-la seria um no-op silencioso -- as 8 tabelas
 * nunca seriam criadas em producao. Consolidar so vale enquanto nada foi
 * aplicado.
 *
 * A tabela `cisternas` do scaffold NAO e derrubada aqui. Ela ficou orfa (o
 * dominio novo nao a usa), mas derrubar tabela no up() de uma migration e
 * destrutivo e irreversivel em producao. A retirada dela e uma migration
 * propria, explicita e revisavel -- ver Task 19.
 *
 * Detalhes do modelo: docs/superpowers/specs/2026-08-10-cisterna-migracao-legado-design.md
 */
return new class extends Migration
{
    ...
    {
        if (Schema::hasTable('cisterna_comunidades')) {
            return;
        }
    ...
    {
        if (Schema::hasTable('cisterna_lotes')) {
            return;
        }
    ...
    {
        if (Schema::hasTable('cisterna_beneficiarios')) {
            return;
        }
    ...
    {
        if (Schema::hasTable('cisterna_atendimentos_pipa')) {
            return;
        }
    ...
    {
        if (Schema::hasTable('cisterna_vistorias')) {
            return;
        }
    ...
    {
        if (Schema::hasTable('cisterna_itens_conferidos')) {
            return;
        }
    ...
    {
        if (Schema::hasTable('cisterna_notificacoes')) {
            return;
        }
