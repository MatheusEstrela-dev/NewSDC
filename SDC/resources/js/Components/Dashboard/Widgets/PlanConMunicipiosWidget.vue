<template>
  <div class="rounded-xl shadow-lg border bg-white dark:bg-slate-800/80 border-slate-100 dark:border-slate-700/50 overflow-hidden h-full group hover:shadow-xl hover:border-blue-500/30 transition-colors duration-300">
    <div class="px-5 py-4 border-b border-slate-100 dark:border-slate-700">
      <h3 class="font-bold text-base text-slate-900 dark:text-slate-200">Plano de Contingencia Municipios Mineiros</h3>
    </div>
      ...
      <div class="flex-1 space-y-3 w-full">
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-slate-700 dark:text-slate-300 font-medium">Municipios com plano</span>
          </div>
          <div class="text-right">
            <span class="text-lg font-bold text-slate-900 dark:text-slate-100">{{ stats.municipiosComPlano }}</span>
            <span class="text-xs text-slate-400 ml-2">{{ stats.percentualComPlano }}%</span>
          </div>
        </div>
        ...
        >
          <div class="flex items-center gap-3">
            ...
            <span class="text-sm text-slate-700 dark:text-slate-300 font-medium">Municipios Sem Plano</span>
          </div>
          <div class="text-right">
            <span class="text-lg font-bold text-slate-900 dark:text-slate-100">{{ stats.municipiosSemPlano }}</span>
            <span class="text-xs text-slate-400 ml-2">{{ (100 - stats.percentualComPlano).toFixed(1) }}%</span>
          </div>
        </div>
      </div>
    </div>
