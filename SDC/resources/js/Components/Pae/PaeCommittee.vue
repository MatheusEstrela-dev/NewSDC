<template>
  <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-in-up">
    <!-- Membros -->
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700/50 overflow-hidden">
      ...
      <div class="p-6 space-y-3">
        <div
          ...
          :key="membro.id"
          class="flex items-center justify-between p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border border-slate-200 dark:border-slate-700/30 hover:border-slate-300 dark:hover:border-slate-600 transition-all"
        >
          <div class="flex items-center gap-4">
            <div
              class="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-700 dark:text-slate-300 font-bold"
            >
              {{ membro.nome.charAt(0) }}
            </div>
            <div>
              <p class="text-slate-900 dark:text-white font-medium">{{ membro.nome }}</p>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700/50 overflow-hidden">
      ...
      <div class="p-6 space-y-4">
        <div
          v-for="ata in atas"
          :key="ata.id"
          class="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-lg border-l-4 border-blue-500 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors cursor-pointer group"
          @click="$emit('view-ata', ata)"
        >
          <div class="flex justify-between items-start">
            <p class="text-base font-medium text-slate-900 dark:text-white group-hover:text-blue-400 transition-colors">
              {{ ata.titulo }}
            </p>
