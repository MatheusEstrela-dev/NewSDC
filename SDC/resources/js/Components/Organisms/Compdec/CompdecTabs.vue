<template>
  <div class="w-full compdec-tabs-container">
    <div class="compdec-tabs-wrapper bg-slate-100 dark:bg-slate-800/30 rounded-xl p-1.5 mb-6 relative">
      <nav class="flex gap-1 overflow-x-auto hide-scrollbar" aria-label="Tabs">
        <button
          ...
          :title="tab.disabled ? (tab.disabledReason || 'Disponivel em breve') : tab.label"
        ...
        >
          <component
            ...
            :class="activeTab === tab.id ? 'text-blue-600 dark:text-blue-400' : ''"
          />
    ...
    <div class="compdec-tabs-content">
...
const props = defineProps({
  ...
  tabs: {
    type: Array,
    required: true,
    // Cada tab: { id: string, label: string, icon?: Component, disabled?: boolean, disabledReason?: string, badge?: number|string }
  },
});
...
<style scoped>
.compdec-tabs-container {
  width: 100%;
  position: relative;
}
.compdec-tabs-wrapper {
  position: relative;
  z-index: 10;
}
.compdec-tabs-content {
  position: relative;
  z-index: 1;
  width: 100%;
}
