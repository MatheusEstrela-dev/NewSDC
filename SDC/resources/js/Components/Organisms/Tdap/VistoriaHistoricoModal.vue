  ...
  <Modal :show="open" max-width="2xl" @close="$emit('close')">
    <div class="flex max-h-full min-h-0 flex-col bg-slate-900 text-slate-200">
      ...
      <div class="flex-1 min-h-0 overflow-y-auto p-4 pb-8 md:p-6">
        <div v-if="carregando" class="py-10 text-center text-slate-400">
        ...
        <div v-else-if="abaAtiva === 'timeline'">
          <ol v-if="total" class="relative border-l border-slate-700 ml-5 space-y-6">
                ...
                <div class="flex items-start justify-between gap-3 mb-2">
                  <div class="min-w-0">
                    ...
                    <div class="mt-1 flex flex-wrap items-center gap-2 text-xs text-slate-400">
                      ...
                      <span class="font-mono text-slate-400">{{ fmtDate(vistoria.data) }}</span>
                  ...
                  <Link
                    ...
                    class="shrink-0 rounded-lg border border-slate-700/60 px-3 py-1 text-xs font-semibold text-slate-200 transition hover:border-blue-400 hover:text-white"
                  >
                    Abrir ficha
                  </Link>
                </div>

                <p v-if="vistoria.observacoes" class="text-sm text-slate-300 leading-relaxed">
                  {{ vistoria.observacoes }}
                </p>

                <div class="mt-3 flex flex-wrap items-center gap-2 text-xs">
                  <span class="inline-flex items-center gap-2 bg-slate-900/40 border border-slate-700/40 px-3 py-1 rounded-full">
                    <UsersIcon class="w-4 h-4 text-slate-400" />
                    <span class="text-slate-400">Vistoriador:</span>
                    <span class="text-slate-200 font-semibold">{{ vistoria.vistoriador || '—' }}</span>
                  </span>
                  <span v-if="vistoria.ficha" class="inline-flex items-center gap-2 bg-slate-900/40 border border-slate-700/40 px-3 py-1 rounded-full">
                    <span class="text-slate-400">Ficha:</span>
                    <span class="font-mono text-slate-200">{{ vistoria.ficha }}</span>
                  </span>
                  <span v-if="vistoria.lacre" class="inline-flex items-center gap-2 bg-slate-900/40 border border-slate-700/40 px-3 py-1 rounded-full">
                    <span class="text-slate-400">Lacre:</span>
                    <span class="font-mono text-slate-200">{{ vistoria.lacre }}</span>
                  </span>
                </div>
              </div>
            </li>
          ...
          <div v-if="reprovadas.length" class="space-y-3">
            ...
            >
              <div class="flex items-start justify-between gap-3">
                <div class="min-w-0">
                  <h4 class="text-base font-semibold text-white">{{ fmtDate(vistoria.data) }}</h4>
                  <p class="text-sm text-slate-400 mt-1">
                    <span class="font-semibold text-slate-300">{{ vistoria.vistoriador || '—' }}</span>
                    <span v-if="vistoria.edital"> • Edital {{ vistoria.edital }}</span>
                  </p>
                  <p v-if="vistoria.observacoes" class="mt-2 text-sm text-slate-300">{{ vistoria.observacoes }}</p>
                </div>
