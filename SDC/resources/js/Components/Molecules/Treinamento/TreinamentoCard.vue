...
const vagasText = computed(() => {
  if (props.treinamento.numeroVagas === null) {
    return 'Vagas ilimitadas';
  }
  return `${props.treinamento.vagasDisponiveis || 0} de ${props.treinamento.numeroVagas} vagas`;
});
</script>
...
<template>
  <CardBase class="treinamento-card hover:shadow-lg transition-all cursor-pointer" @click="emit('view', treinamento.id)">
    ...
    <div class="flex justify-between items-start mb-3">
      ...
      <Badge :color="treinamento.statusColor" class="ml-2">
    ...
    <div class="space-y-2 mb-4">
      ...
      <!-- Datas -->
      <div v-if="treinamento.dataInicio" class="flex items-center gap-2">
        <svg class="w-4 h-4 text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
        </svg>
        <Text size="sm">
          {{ formatDate(treinamento.dataInicio) }}
          <span v-if="treinamento.dataFim"> até {{ formatDate(treinamento.dataFim) }}</span>
        </Text>
      </div>
    </div>
    ...
    <div class="flex justify-between items-center pt-3 border-t border-slate-200 dark:border-slate-700">
      <div class="flex gap-4">
        <Text size="xs" color="muted" class="flex items-center gap-1">
          ...
          {{ treinamento.totalInscricoes }} inscritos
        </Text>
        <Text size="xs" color="muted" class="flex items-center gap-1">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
          </svg>
          {{ treinamento.totalModulos }} módulos
        </Text>
