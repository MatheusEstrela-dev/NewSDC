...
const form = useForm({
  titulo: props.treinamento.titulo ?? '',
  descricao: props.treinamento.descricao ?? '',
  carga_horaria: props.treinamento.carga_horaria ?? '',
  categoria: props.treinamento.categoria ?? 'CURSO',
  tipo: props.treinamento.tipo ?? 'PRESENCIAL',
  instrutor: props.treinamento.instrutor ?? '',
  local: props.treinamento.local ?? '',
  data_inicio: props.treinamento.data_inicio ?? '',
  data_fim: props.treinamento.data_fim ?? '',
  hora_inicio: props.treinamento.hora_inicio ?? '',
  percentual_frequencia_minimo: props.treinamento.percentual_frequencia_minimo ?? 75,
  presenca_autoconfirmavel: props.treinamento.presenca_autoconfirmavel ?? false,
});

function salvar() {
  form.put(route('treinamentos.update', props.treinamento.id), {
    onError: () => toast('Verifique os campos do formulario.', 'error'),
  });
}

function cancelar() {
  router.visit(route('treinamentos.show', props.treinamento.id));
}
</script>
