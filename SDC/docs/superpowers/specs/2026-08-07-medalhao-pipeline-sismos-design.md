...
### 4.5 Leaflet nesta fase; MapLibre avaliado na fase 5

Avaliamos trocar Leaflet por Mapbox. Os ganhos tecnicos sao reais e relevantes
para o roadmap: renderizacao em GPU (Leaflet degrada com milhares de marcadores,
o que importa para os pluviometros do CEMADEN), heatmap e styling orientado a
dado nativos, e melhor suporte a overlay raster — que e exatamente a forma da
superficie de krigagem da fase 5.

Dois fatores decidem contra o Mapbox neste sistema:

- **Licenca e custo.** O Mapbox GL JS deixou de ser open source na v2: e
  proprietario, exige access token e cobra por map load acima da faixa gratuita.
  Em orgao publico isso implica processo de compra e custo que cresce com o uso.
- **On-premise.** O projeto saiu do Azure para on-premise. Mapbox e SaaS: cada
  carregamento busca tile no servidor deles, atravessando o proxy corporativo.
  Numa ferramenta de emergencia, isso condiciona a renderizacao do mapa ao link
  externo.

`maplibre-gl` (fork BSD-3 do Mapbox GL JS v1) entrega as mesmas capacidades sem
token, sem cobranca por load e podendo apontar para tiles auto-hospedados.

Decisao: fase 1 permanece em Leaflet, no padrao de `MapaInmet.vue` — sao dezenas
de eventos, que nao estressam nada, e trocar agora criaria inconsistencia com o
mapa do INMET. A migracao para MapLibre recebe spec proprio quando a fase 5
introduzir o raster e a fase 2 o volume de pontos, cobrindo os dois mapas juntos.

### 4.6 Sem `Bus::batch` nesta fase
