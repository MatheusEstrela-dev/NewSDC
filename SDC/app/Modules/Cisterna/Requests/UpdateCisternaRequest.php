...
class UpdateCisternaRequest extends FormRequest
{
    ...
    {
        $cisterna = $this->route('cisterna');

        return $this->user()?->can('update', $cisterna) ?? false;
    }
    ...
    {
        $cisternaId = $this->route('cisterna')?->id ?? null;

        return [
            'municipio_id' => ['required', 'integer', Rule::exists('municipios', 'id')],
            'codigo' => [
                'required', 'string', 'max:60',
                Rule::unique('cisternas', 'codigo')->ignore($cisternaId),
            ],
