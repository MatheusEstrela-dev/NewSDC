    ...
    private function copyFilesWithProgress($source, $destination, $excludedDirs = []): void
    {
        ...
        if (PHP_OS_FAMILY === 'Windows') {
            ...
            try {
                ...
                // $this->components->warn("Rsync failed, falling back to PHP copy...");
                $this->recursiveCopy($source, $destination, $excludedDirs);
            }
        }
    }

    private function recursiveCopy($source, $destination, $excludedDirs = [])
    {
        $dir = opendir($source);
        @mkdir($destination, 0755, true);
        while(false !== ( $file = readdir($dir)) ) {
            if (( $file != '.' ) && ( $file != '..' )) {
                ...
                foreach ($excludedDirs as $excluded) {
                     // Check against full relative path logic would be better but simple check:
                     if ($file === $excluded || strpos($fullPath, $excluded) !== false) continue 2;
                }

                if ( is_dir($fullPath) ) {
                    $this->recursiveCopy($fullPath, $destPath, $excludedDirs);
                }
                else {
